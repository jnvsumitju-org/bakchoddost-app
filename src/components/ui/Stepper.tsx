export default function Stepper({ steps, active }: { steps: string[]; active: number }) {
  return (
    <ol className="flex flex-wrap items-center gap-3 text-xs sm:text-sm">
      {steps.map((label, i) => {
        const idx = i + 1;
        ...
        return (
          <li key={label} className="flex items-center gap-2">
            <span
              className={`h-6 w-6 inline-flex items-center justify-center rounded-full border text-[10px] sm:text-xs ${
            ...
            {i !== steps.length - 1 && <span className="w-6 sm:w-8 h-px bg-border mx-1 sm:mx-2" />}
          </li>
        );
      })}
    </ol>
